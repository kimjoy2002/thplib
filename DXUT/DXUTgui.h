...
#define EVENT_RBUTTON_CLICKED				0x0102
...
//> mod
#define EVENT_SCROLLBAR_POS_CHANGE		0x0800
//< mod
...
//> mod
class DXUTControlLock;
class CDXUTColorListBox;
class CDXUTMeter;
class CDXUTAngleLine;
class CDXUTSprite;
class CDXUTAnimationSprite;
//< mod
...
//> mod
	DXUT_CONTROL_COLORLISTBOX,
	DXUT_CONTROL_METER,
	DXUT_CONTROL_ANGLELINE,
	DXUT_CONTROL_SPRITE,
	DXUT_CONTROL_ANIMATION_SPRITE,
//< mod
};
...
//> mod
	HRESULT AddColorListBox( int ID, int x, int y, int width, int height, DWORD dwStyle, CDXUTColorListBox** ppCreated );
    CDXUTColorListBox*     GetColorListBox( int ID ) { return (CDXUTColorListBox*) GetControl( ID, DXUT_CONTROL_COLORLISTBOX ); }
	HRESULT AddMeter( int ID, int val, int max_val, int x, int y, int width, int height, bool bIsDefault, CDXUTMeter** ppCreated );
	CDXUTMeter*     GetMeter( int ID ) { return (CDXUTMeter*) GetControl( ID, DXUT_CONTROL_METER ); };
	HRESULT AddAngleLine( int ID, int angle, int length, int x, int y, int width, int height, bool bIsDefault, CDXUTAngleLine** ppCreated );
	CDXUTAngleLine*     GetAngleLine( int ID ) { return (CDXUTAngleLine*) GetControl( ID, DXUT_CONTROL_ANGLELINE ); };
	HRESULT AddSprite( int ID, int x, int y, int width, int height, bool bIsDefault, CDXUTSprite** ppCreated );
	CDXUTSprite*     GetSprite( int ID ) { return (CDXUTSprite*) GetControl( ID, DXUT_CONTROL_SPRITE ); };
	HRESULT AddAnimationSprite( int ID, int x, int y, int width, int height, bool bIsDefault, BOOL bLoop, CDXUTAnimationSprite** ppCreated );
	CDXUTAnimationSprite*     GetAnimationSprite( int ID ) { return (CDXUTAnimationSprite*) GetControl( ID, DXUT_CONTROL_ANIMATION_SPRITE ); };
	HRESULT AddScrollBar( int ID, int x, int y, int width, int height, int min=0, int max=100, int value=50, bool bIsDefault=false, CDXUTScrollBar** ppCreated=NULL );
	CDXUTScrollBar*     GetScrollBar( int ID ) { return (CDXUTScrollBar*) GetControl( ID, DXUT_CONTROL_SCROLLBAR ); };
//< mod
	...
	CGrowableArray< DXUTElementHolder* >* GetDefaultElement() { return &m_DefaultElements; };
...
//> mod
	HRESULT DrawSprite(ID3DXSprite* pSprite, CDXUTElement* pElement, CDXUTDialog* pDialog, RECT* prcDest, D3DXMATRIX* mat=NULL);
	HRESULT DrawText(ID3DXSprite* pSprite, LPCWSTR strText, CDXUTElement* pElement, CDXUTDialog* pDialog, RECT* prcDest, bool bShadow = false, int nCount = -1, D3DXMATRIX* mat=NULL);
//< mod
    // Attributes
...
//> mod
    virtual void Render( ID3DXSprite* pSprite, IDirect3DDevice9* pd3dDevice, float fElapsedTime, D3DXMATRIX* mat) { };
//< mod
    // Windows message handler
...
public:
    ...
    virtual void Render( IDirect3DDevice9* pd3dDevice, float fElapsedTime );
//> mod
    virtual void Render(ID3DXSprite* pSprite, IDirect3DDevice9* pd3dDevice, CDXUTDialog* pDialog, float fElapsedTime, D3DXMATRIX* mat );
//< mod
	...
	bool GetDrawShadow()	{ return m_bDrawShadow;	};
	void SetDrawShadow(bool value)	{ m_bDrawShadow = value;	};
...
//> mod
	...
	virtual void SetStateMouserOverOffset(int nOffsetX, int nOffsetY)
	{
		m_nMouserOverOffsetX = nOffsetX;
		m_nMouserOverOffsetY = nOffsetY;
	};
	virtual void SetStatePressedOverOffset(int nOffsetX, int nOffsetY)
	{
		m_nPressedOffsetX = nOffsetX;
		m_nPressedOffsetY = nOffsetY;
	};
	virtual bool GetEnabledRightClickEvent() { return m_bEnabledRightClickEvent;	};
	virtual void SetEnabledRightClickEvent(bool value) { m_bEnabledRightClickEvent = value; };
//< mod
protected:
    bool m_bPressed;
	int m_nMouserOverOffsetX;
	int m_nMouserOverOffsetY;
	int m_nPressedOffsetX;
	int m_nPressedOffsetY;
//> mod
	bool m_bEnabledRightClickEvent;
//< mod
};
...
public:
	...
	bool GetHandleKeyboardEnabled()	{ return m_bHandleKeyboardEnabled;	};
	void SetHandleKeyboardEnabled(bool bEnabled)	{ m_bHandleKeyboardEnabled = bEnabled;	};
protected:
...
//> mod
	...
	bool GetDrawTextShadow() { return m_bDrawTextShadow;	};
	void SetDrawTextShadow(bool value) { m_bDrawTextShadow = value;	};
//< mod
    int  GetScrollBarWidth() const { return m_nSBWidth; }
    void SetScrollBarWidth( int nWidth ) { m_nSBWidth = nWidth; UpdateRects(); }
    void SetBorder( int nBorder, int nMargin ) { m_nBorder = nBorder; m_nMargin = nMargin; UpdateRects(); }
...
//> mod
	bool m_bDrawTextShadow;
//< mod

    CGrowableArray< DXUTListBoxItem* > m_Items;
};

struct DXUTColorListBoxItem : public DXUTListBoxItem
{
	int	nResourceIndex;
};
//> mod
class CDXUTColorListBox : public CDXUTListBox
{
public:
	CDXUTColorListBox( CDXUTDialog *pDialog = NULL ):CDXUTListBox(pDialog)
	{
		SetChangeSelectColor(true);
		m_bAcceptKeyboard = true;
		m_Type = DXUT_CONTROL_COLORLISTBOX;
	};
    virtual ~CDXUTColorListBox()	
	{    RemoveAllItems();	};
/*
    virtual HRESULT OnInit() { return m_pDialog->InitControl( &m_ScrollBar ); }
    virtual bool    CanHaveFocus() { return (m_bVisible && m_bEnabled); }
...
*/
    ...
    void RemoveAllItems();
    virtual void    UpdateRects();
	void SetChangeSelectColor(bool b)
	{	m_bChangeSelectColor = b;	};
    HRESULT AddItem( const WCHAR *wszText, void*pData, int nResourceIndex );
    HRESULT InsertItem( int nIndex, const WCHAR *wszText, void *pData, int nResourceIndex );
    virtual void    Render( IDirect3DDevice9* pd3dDevice, float fElapsedTime );
//	virtual int GetSelectedIndex( int nPreviousSelected );
//	virtual bool HandleKeyboard( UINT uMsg, WPARAM wParam, LPARAM lParam );
//	virtual bool HandleMouse( UINT uMsg, POINT pt, WPARAM wParam, LPARAM lParam );

	bool m_bAcceptKeyboard;
protected:
/*
    RECT m_rcText;      // Text rendering bound
    RECT m_rcSelection; // Selection box bound
    ...
    CGrowableArray< DXUTListBoxItem* > m_Items;
*/
    RECT m_rcDraw;      // Text rendering bound
	bool m_bChangeSelectColor;
};
//< mod
...
//-----------------------------------------------------------------------------
enum E_METER_STYLE {
	CDXUTMeterStyleLeftToRight,
	CDXUTMeterStyleBottomToTop,
};
class CDXUTMeter : public CDXUTControl
{
public:
    CDXUTMeter( CDXUTDialog *pDialog = NULL );
...
protected:
	...
	RECT m_rcValue;
	RECT m_rcDraw;
	RECT m_rcValueTexture;

	E_METER_STYLE m_eMeterStyle;
};
//< mod
...
// AngleLine control
//-----------------------------------------------------------------------------
class CDXUTAngleLine : public CDXUTControl
{
public:
	CDXUTAngleLine( CDXUTDialog *pDialog = NULL );
	...
	void SetAngle(int value) {m_nAngle = (value%360);	UpdateRects(); };
	// ベクトルから設定
	void SetAngle(int x,int y);

	int GetLength() { return m_nLength; };
	void SetLength(int value) { m_nLength = value; UpdateRects(); };
	
	D3DCOLOR GetLineColor() { return m_clrLineColor;	};
	void SetLineColor(D3DCOLOR value) { m_clrLineColor = value;	};

protected:
	int m_nLength;
	int m_nAngle;
	D3DCOLOR m_clrLineColor;
	struct {
		POINT m_pntStart;
		POINT m_pntEnd;
	} line;
...
// Sprite control
//-----------------------------------------------------------------------------
class CDXUTSprite : public CDXUTControl
{
public:
	CDXUTSprite( CDXUTDialog *pDialog = NULL );
	virtual void Render( IDirect3DDevice9* pd3dDevice, float fElapsedTime );
	virtual void Render( ID3DXSprite* pSprite, IDirect3DDevice9* pd3dDevice, CDXUTDialog* pDialog, float fElapsedTime, D3DXMATRIX* mat);
	virtual BOOL ContainsPoint( POINT pt ) { return false; };
protected:
...
//> mod
//-----------------------------------------------------------------------------
// AnimationSprite control
//-----------------------------------------------------------------------------
class CDXUTAnimationSprite : public CDXUTSprite
{
public:
	CDXUTAnimationSprite( CDXUTDialog *pDialog = NULL );
	virtual void Render( IDirect3DDevice9* pd3dDevice, float fElapsedTime );
	virtual BOOL ContainsPoint( POINT pt ) { return false; };
	
	virtual void AddAnimationRect(const RECT* rc)
	{
		m_vecRcTexture.push_back(*rc);
		m_nAnimationCount = m_vecRcTexture.size();
	};

	virtual void SetAnimationTime(int value)
	{
		m_nAnimationTime = value;
		UpdateAnimation();
	};
	virtual void SetAnimationIndex(int value)
	{
		m_nAnimationIndex = min(m_nAnimationCount, value);
		UpdateAnimation();
		CDXUTElement* pElement = m_Elements.GetAt( 0 );
		if (!pElement) return;
		CopyRect(&pElement->rcTexture, &m_vecRcTexture[m_nAnimationIndex]);
	};
	virtual BOOL UpdateAnimation()
	{
		BOOL ret = FALSE;
		if (m_nAnimationTime <= m_nAnimationTimeCounter)
		{
			...
			else if ( (m_nAnimationIndex+1) < m_nAnimationCount)
			{
				m_nAnimationIndex = m_nAnimationIndex+1;
				ret = TRUE;
			}
		}
		return ret;
	...
	virtual void StepTimeCount(int value=1)
	{
		if (m_bEnabled)
		{
			m_nAnimationTimeCounter += value;
			if (UpdateAnimation())
			{
				RECT rcDraw = m_vecRcTexture[m_nAnimationIndex];
				CopyRect(&m_Elements.GetAt( 0 )->rcTexture, &rcDraw);
			}
		}
	};

	virtual int GetAnimationTimeCounter(){ return m_nAnimationTimeCounter;	};
	virtual int GetAnimationTime(){ return m_nAnimationTime;	};
	virtual int GetAnimationIndex(){ return m_nAnimationIndex;	};
	virtual int GetAnimationCount(){ return m_nAnimationCount;	};
	virtual BOOL IsLoop()	{ return m_bLoop;	};
	virtual void SetIsLoop(BOOL value) { m_bLoop = value; };

	virtual BOOL GetAutoCountOnRender() { return m_bAutoCountOnRender;	};
	virtual void SetAutoCountOnRender(BOOL value) { m_bAutoCountOnRender = value;	};

	virtual void InitAnimationTime()
	{
		m_nAnimationTimeCounter = 0;
		m_nAnimationIndex = 0;
		CDXUTElement* pElement = m_Elements.GetAt( 0 );
	...
	virtual void Clear()
	{
		...
		m_nAnimationIndex = 0;
		m_nAnimationCount = 0;
		m_vecRcTexture.clear();
	};

protected:
	
	int m_nAnimationTimeCounter;
	int m_nAnimationTime;

	int m_nAnimationIndex;
	int m_nAnimationCount;

	std::vector<RECT> m_vecRcTexture;

	BOOL m_bAutoCountOnRender;
	BOOL m_bLoop;
...
private:
	bool* p_pThreadSafe;
	CRITICAL_SECTION* p_pCS;  
public:
	inline DXUTControlLock(bool *bThreadSafe, CRITICAL_SECTION* p_cs)
	{
		if( *bThreadSafe )
		{
			p_pThreadSafe = bThreadSafe;
			p_pCS = p_cs;
	...
	inline void DXUTControlLockDestroy(bool *bThreadSafe, CRITICAL_SECTION* p_cs) {
		*bThreadSafe = false;
		LeaveCriticalSection( p_cs );
		DeleteCriticalSection( p_cs );
	}
	inline ~DXUTControlLock() { if( *p_pThreadSafe ) LeaveCriticalSection( p_pCS ); }
};
//< mod 20100928
...
static CRITICAL_SECTION s_IMEEditBoxCS;  
static bool s_bIMEEditBoxThreadSafe;
...
//< mod
//> mod Reading,Candidateの色を設定できるようにする
	inline void SetReadingColor(D3DCOLOR value) { m_ReadingColor = value;	};
	inline void SetReadingWinColor(D3DCOLOR value) { m_ReadingWinColor = value;	};
	inline void SetReadingSelColor(D3DCOLOR value) { m_ReadingSelColor = value;	};
	inline void SetReadingSelBkColor(D3DCOLOR value) { m_ReadingSelBkColor = value;	};
	inline void SetCandidateColor(D3DCOLOR value) { m_CandidateColor = value;	};
	inline void SetCandidateWinColor(D3DCOLOR value) { m_CandidateWinColor = value;	};
	inline void SetCandidateSelColor(D3DCOLOR value) { m_CandidateSelColor = value;	};
	inline void SetCandidateSelBkColor(D3DCOLOR value) { m_CandidateSelBkColor = value;	};
//< mod
protected:
