...
int CDXUTDialogResourceManager::AddTexture( LPCWSTR strFilename )
{
    ...
    for( int i=0; i < m_TextureCache.GetSize(); i++ )
    {
        ...
        size_t nInLen = 0;
		size_t nNodeLen = 0;
        StringCchLength( strFilename, MAX_PATH, &nInLen );        
        StringCchLength( pTextureNode->strFilename, MAX_PATH, &nNodeLen );        
        if( pTextureNode->bFileSource &&  // Sources must match
			nInLen == nNodeLen &&
            0 == _wcsnicmp( pTextureNode->strFilename, strFilename, nInLen ) )
            return i;
    }
...
//> mod
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::AddMeter( int ID, int val, int max_val, int x, int y, int width, int height, bool bIsDefault, CDXUTMeter** ppCreated )
{
    HRESULT hr = S_OK;

    CDXUTMeter* pMeter = new CDXUTMeter( this );
    ...
    hr = AddControl( pMeter );
    if( FAILED(hr) )
        return hr;

    // Set the ID and list index
    pMeter->SetID( ID ); 
    pMeter->SetLocation( x, y );
    pMeter->SetSize( width, height );
    pMeter->m_bIsDefault = bIsDefault;
	pMeter->SetMaxValue(max_val);
	pMeter->SetValue(val);
...
//> mod
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::AddAngleLine( int ID, int angle, int length, int x, int y, int width, int height, bool bIsDefault, CDXUTAngleLine** ppCreated )
{
    HRESULT hr = S_OK;

    CDXUTAngleLine* pAngleLine = new CDXUTAngleLine( this );

    if( ppCreated != NULL )
        *ppCreated = pAngleLine;

    if( pAngleLine == NULL )
        return E_OUTOFMEMORY;

    hr = AddControl( pAngleLine );
    if( FAILED(hr) )
        return hr;

    // Set the ID and list index
    pAngleLine->SetID( ID ); 
    pAngleLine->SetLocation( x, y );
    pAngleLine->SetSize( width, height );
    pAngleLine->m_bIsDefault = bIsDefault;
	pAngleLine->SetLength(length);
	pAngleLine->SetAngle(angle);
...
//> mod
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::AddSprite( int ID, int x, int y, int width, int height, bool bIsDefault, CDXUTSprite** ppCreated )
{
    HRESULT hr = S_OK;

    CDXUTSprite* pSprite = new CDXUTSprite( this );
    ...
    hr = AddControl( pSprite );
    if( FAILED(hr) )
        return hr;

    // Set the ID and list index
    pSprite->SetID( ID ); 
    pSprite->SetLocation( x, y );
    pSprite->SetSize( width, height );
    pSprite->m_bIsDefault = bIsDefault;
...
//> mod
HRESULT CDXUTDialog::AddScrollBar( int ID, int x, int y, int width, int height, int min, int max, int value, bool bIsDefault, CDXUTScrollBar** ppCreated )
{
    HRESULT hr = S_OK;
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::AddColorListBox( int ID, int x, int y, int width, int height, DWORD dwStyle, CDXUTColorListBox** ppCreated )
{
    HRESULT hr = S_OK;
    CDXUTColorListBox *pListBox = new CDXUTColorListBox( this );
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::AddAnimationSprite( int ID, int x, int y, int width, int height, bool bIsDefault, BOOL bLoop, CDXUTAnimationSprite** ppCreated )
{
    HRESULT hr = S_OK;
    CDXUTAnimationSprite *pAnimationSprite = new CDXUTAnimationSprite( this );

    if( ppCreated != NULL )
        *ppCreated = pAnimationSprite;

    if( pAnimationSprite == NULL )
        return E_OUTOFMEMORY;

    hr = AddControl( pAnimationSprite );
    if( FAILED(hr) )
        return hr;

    // Set the ID and position
    pAnimationSprite->SetID( ID );
    pAnimationSprite->SetLocation( x, y );
    pAnimationSprite->SetSize( width, height );
	pAnimationSprite->SetIsLoop(bLoop);
...
//> mod
	m_pManager->m_pSprite->SetTransform( &(m_matTransform*matTransform) );
...
//> mod
HRESULT CDXUTDialog::DrawSprite(ID3DXSprite* pSprite, CDXUTElement* pElement, CDXUTDialog* pDialog, RECT* prcDest , D3DXMATRIX* mat)
{
...
//    OffsetRect( &rcScreen, m_x, m_y );
    ...
    // If caption is enabled, offset the Y position by its height.
	if( pDialog->m_bCaption )
        OffsetRect( &rcScreen, 0, pDialog->GetCaptionHeight() );

    DXUTTextureNode* pTextureNode = pDialog->GetTexture( pElement->iTexture );
    if( pTextureNode == NULL )
        return E_FAIL;
...
//> mod
	if (!mat)
	    ...
	    pSprite->SetTransform( &(matTransform*(*mat)) );
//    m_pManager->m_pSprite->SetTransform( &matTransform );
//< mod    
    D3DXVECTOR3 vPos( (float)rcScreen.left, (float)rcScreen.top, 0.0f );
    ...
    return pSprite->Draw( pTextureNode->pTexture, &rcTexture, NULL, &vPos, pElement->TextureColor.Current );
}
//< mod
...
//        hr = pFontNode->pFont->DrawText( m_pManager->m_pSprite, strText, nCount, &rcShadow, pElement->dwTextFormat, D3DCOLOR_ARGB(DWORD(pElement->FontColor.Current.a * 255), 0, 0, 0) );
        hr = pFontNode->pFont->DrawText( m_pManager->m_pSprite, strText, nCount, &rcShadow, pElement->dwTextFormat, D3DCOLOR_ARGB(DWORD(pElement->FontColor.Current.a * 255.0F), 0, 0, 0) );
//< 20100927 mod
        if( FAILED(hr) )
            return hr;
    }
...
//> mod
HRESULT CDXUTDialog::DrawText(ID3DXSprite* pSprite, LPCWSTR strText, CDXUTElement* pElement, CDXUTDialog* pDialog, RECT* prcDest, bool bShadow, int nCount, D3DXMATRIX* mat)
{
	if (!pSprite) return E_FAIL;
    HRESULT hr = S_OK;
...
//> mod
	if (!mat)
	    ...
	    pSprite->SetTransform( &((*mat) * matTransform) );
//    m_pManager->m_pSprite->SetTransform( &matTransform );
//< mod
    DXUTFontNode* pFontNode = pDialog->GetFont(pElement->iFont);
    
    if( bShadow )
    {
        RECT rcShadow = rcScreen;
        OffsetRect( &rcShadow, 1, 1 );
        hr = pFontNode->pFont->DrawText( pSprite, strText, nCount, &rcShadow, pElement->dwTextFormat, D3DCOLOR_ARGB(DWORD(pElement->FontColor.Current.a * 255), 0, 0, 0) );
        if( FAILED(hr) )
            return hr;
    }

    hr = pFontNode->pFont->DrawText( pSprite, strText, nCount, &rcScreen, pElement->dwTextFormat, pElement->FontColor.Current );
    if( FAILED(hr) )
        return hr;
...
void CDXUTDialog::InitDefaultElements()
{
    ...
    SetRect( &rcTexture, 0, 0, 136, 54 );
    Element.SetTexture( 0, &rcTexture );
    Element.SetFont( 0, D3DCOLOR_ARGB( 255, 255, 0, 0 ), DT_CENTER | DT_VCENTER );
...
//> mod
    ...
    // Assign the Element
    SetDefaultElement( DXUT_CONTROL_COLORLISTBOX, 0, &Element );

    //-------------------------------------
    // CDXUTListBox - Default 1
    //-------------------------------------
    ...
    // Assign the Element
    SetDefaultElement( DXUT_CONTROL_COLORLISTBOX, 1, &Element );

    //-------------------------------------
    // CDXUTListBox - Default 2
    //-------------------------------------

    SetRect( &rcTexture, 16, 166, 240, 183 );
    Element.SetTexture( 0, &rcTexture );
    Element.SetFont( 0, D3DCOLOR_ARGB(255, 0, 255, 255), DT_LEFT | DT_TOP );

    // Assign the Element
    SetDefaultElement( DXUT_CONTROL_COLORLISTBOX, 2, &Element );

    //-------------------------------------
    // CDXUTListBox - Default 3
    //-------------------------------------

    SetRect( &rcTexture, 16, 166, 240, 183 );
    Element.SetTexture( 0, &rcTexture );
    Element.SetFont( 0, D3DCOLOR_ARGB(255, 255, 255, 0), DT_LEFT | DT_TOP );

    // Assign the Element
    SetDefaultElement( DXUT_CONTROL_COLORLISTBOX, 3, &Element );

	D3DXMatrixIdentity(&m_matTransform);
//< mod
//> mod
    //-------------------------------------
    // CDXUTMeter - Fill layer
    //-------------------------------------
    SetRect( &rcTexture, 0, 0, 136, 54 );
    Element.SetTexture( 0, &rcTexture );
    Element.SetFont( 0 );    
    // Assign the Element
    SetDefaultElement( DXUT_CONTROL_METER, 0, &Element );
    ...
    // Assign the Element
    SetDefaultElement( DXUT_CONTROL_METER, 1, &Element );

	//-------------------------------------
    // CDXUTMeter - Front Frame
    //-------------------------------------
    SetRect( &rcTexture, 136, 0, 252, 54 );
    Element.SetTexture( 0, &rcTexture, D3DCOLOR_ARGB(0, 255, 255, 255) );    
    
    // Assign the Element
    SetDefaultElement( DXUT_CONTROL_METER, 2, &Element );
...
//> mod
    ...
    Element.SetFont( 0 );    
    // Assign the Element
    SetDefaultElement( DXUT_CONTROL_ANGLELINE, 0, &Element );
//< mod
//> mod
    //-------------------------------------
    // CDXUTSprite - Fill layer
    //-------------------------------------
    SetRect( &rcTexture, 0, 0, 136, 54 );
    Element.SetTexture( 0, &rcTexture );
    Element.SetFont( 0 );
    // Assign the Element
    SetDefaultElement( DXUT_CONTROL_SPRITE, 0, &Element );
//< mod
//> mod
    //-------------------------------------
    // CDXUTAnimationSprite - Fill layer
    //-------------------------------------
    SetRect( &rcTexture, 0, 0, 136, 54 );
    Element.SetTexture( 0, &rcTexture );
    Element.SetFont( 0 );
    // Assign the Element
    SetDefaultElement( DXUT_CONTROL_ANIMATION_SPRITE, 0, &Element );
//< mod
...
//--------------------------------------------------------------------------------------
//> mod
void CDXUTStatic::Render(ID3DXSprite* pSprite, IDirect3DDevice9* pd3dDevice, CDXUTDialog* pDialog, float fElapsedTime, D3DXMATRIX* mat )
{
	if (!pSprite) return;
    if( m_bVisible == false )
        return;
    ...
    pDialog->DrawTextW(pSprite, m_strText, pElement, pDialog, &m_rcBoundingBox, m_bDrawShadow, -1, mat );
}
//< mod
...
void CDXUTStatic::Render( IDirect3DDevice9* pd3dDevice, float fElapsedTime )
{
    ...
    pElement->FontColor.Blend( iState, fElapsedTime );
//> mod
    m_pDialog->DrawText( m_strText, pElement, &m_rcBoundingBox, m_bDrawShadow );
...
CDXUTButton::CDXUTButton( CDXUTDialog *pDialog )
{
    ...
    m_nHotkey = 0;
//> mod
	m_nMouserOverOffsetX = -1;
	m_nMouserOverOffsetY = -1;
	m_nPressedOffsetX = 1;
	m_nPressedOffsetY = 1;
	m_bEnabledRightClickEvent = false;
//< mod
}
...
void CDXUTButton::Render( IDirect3DDevice9* pd3dDevice, float fElapsedTime )
{
    ...
    else if( m_bPressed )
    {
        ...
        nOffsetX = m_nPressedOffsetX;
//> mod
//        nOffsetY = 2;
        nOffsetY = m_nPressedOffsetY;
//< mod
    }
    else if( m_bMouseOver )
    {
        iState = DXUT_STATE_MOUSEOVER;
        nOffsetX = m_nMouserOverOffsetX;
//> mod
//        nOffsetY = -2;
        nOffsetY = m_nMouserOverOffsetY;
//< mod
    }
    else if( m_bHasFocus )
    {
...
//> mod
void CDXUTButton::Render(ID3DXSprite* pSprite, IDirect3DDevice9* pd3dDevice, CDXUTDialog* pDialog, float fElapsedTime, D3DXMATRIX* mat )
{
	if (!pSprite) return;
...
//< mod
    ...
    pDialog->DrawSprite( pSprite, pElement, pDialog, &rcWindow, mat );
    pDialog->DrawTextW(pSprite, m_strText, pElement, pDialog, &rcWindow, m_bDrawShadow, -1, mat );
...
//    m_pDialog->DrawText( m_strText, pElement, &m_rcText, true );
	m_pDialog->DrawText( m_strText, pElement, &m_rcText, m_bDrawShadow );
//< mod 20100910
...
//    m_rcDropdown.right -= m_nSBWidth;
	RECT rWin = DXUTGetWindowClientRect();
	if (rWin.bottom <= m_rcDropdown.bottom)
		OffsetRect( &m_rcDropdown, 0, (int) -RectHeight( m_rcDropdown ) - (0.90f * RectHeight( m_rcText ))  );
	//< mod
...
//		m_rcThumb.bottom = m_rcThumb.top + RectHeight( m_rcTrack );
        m_bShowThumb = true;
    } 
    else
    {
...
void CDXUTScrollBar::Scroll( int nDelta )
{
    ...
    UpdateThumbRect();
//> mod
 	if (nSavedPos != m_nPosition)
			m_pDialog->SendEvent(EVENT_SCROLLBAR_POS_CHANGE, true, this);
//< mod
}
...
void CDXUTListBox::UpdateRects()
{
    ...
    if( pFontNode && pFontNode->nHeight )
    {
		m_ScrollBar.SetPageSize( (RectHeight( m_rcText)-m_nMargin*2) / (pFontNode->nHeight+(m_nBorder? m_nBorder:0)));
...
//                m_pDialog->DrawText( pItem->strText, pSelElement, &rc );
                m_pDialog->DrawText( pItem->strText, pSelElement, &rc, m_bDrawTextShadow );
//< mod
            }
            else
//> mod
//                m_pDialog->DrawText( pItem->strText, pElement, &rc );
                m_pDialog->DrawText( pItem->strText, pElement, &rc, m_bDrawTextShadow );
//< mod
...
void CDXUTColorListBox::UpdateRects()
{
	CDXUTListBox::UpdateRects();
	m_rcDraw = m_rcSelection;
}

HRESULT CDXUTColorListBox::AddItem( const WCHAR *wszText, void*pData, int nResourceIndex )
{
    DXUTColorListBoxItem *pNewItem = new DXUTColorListBoxItem;
    if( !pNewItem )
        return E_OUTOFMEMORY;
    ...
    SetRect( &pNewItem->rcActive, 0, 0, 0, 0 );
    pNewItem->bSelected = false;
	pNewItem->nResourceIndex = nResourceIndex;
...
HRESULT CDXUTColorListBox::InsertItem( int nIndex, const WCHAR *wszText, void *pData, int nResourceIndex )
{
    DXUTColorListBoxItem *pNewItem = new DXUTColorListBoxItem;
    if( !pNewItem )
        return E_OUTOFMEMORY;
...
//--------------------------------------------------------------------------------------
void CDXUTColorListBox::RemoveItem( int nIndex )
{
    if( nIndex < 0 || nIndex >= (int)m_Items.GetSize() )
        return;

    DXUTColorListBoxItem *pItem = (DXUTColorListBoxItem*)m_Items.GetAt( nIndex );
...
//--------------------------------------------------------------------------------------
void CDXUTColorListBox::RemoveAllItems()
{
    for( int i = 0; i < m_Items.GetSize(); ++i )
    {
        DXUTColorListBoxItem *pItem = (DXUTColorListBoxItem*)m_Items.GetAt( i );
        delete pItem;
    }
...
//--------------------------------------------------------------------------------------
void CDXUTColorListBox::Render( IDirect3DDevice9* pd3dDevice, float fElapsedTime )
{
    if( m_bVisible == false )
        return;
    ...
    pElement->TextureColor.Blend( DXUT_STATE_NORMAL, fElapsedTime );
    pElement->FontColor.Blend( DXUT_STATE_NORMAL, fElapsedTime );
	int nHOffset = m_nTextHeight + m_nBorder;

    CDXUTElement* pSelElement = m_Elements.GetAt( 0 );
    pSelElement->TextureColor.Blend( DXUT_STATE_NORMAL, fElapsedTime );
    pSelElement->FontColor.Blend( DXUT_STATE_NORMAL, fElapsedTime );

	for (int i=2;i<m_Elements.GetSize();i++)
	{
		CDXUTElement* pColorElement = m_Elements.GetAt(i);
		pColorElement->TextureColor.Blend( DXUT_STATE_NORMAL, fElapsedTime );
		pColorElement->FontColor.Blend( DXUT_STATE_NORMAL, fElapsedTime );
	}
    ...
    if( m_Items.GetSize() > 0 )
    {
        ...
        for( int i = m_ScrollBar.GetTrackPos(); i < (int)m_Items.GetSize(); ++i )
        {
			...
			if (m_bChangeSelectColor)
			{
				...
				if( bSelectedStyle )
				{
					CDXUTElement* pColorElement = m_Elements.GetAt(pItem->nResourceIndex);
					rcSel.top = rc.top; rcSel.bottom = rc.bottom;
					rcDraw.left = m_rcDraw.left;
					rcDraw.top = rcSel.top;
					rcDraw.right = rcDraw.left+(pColorElement->rcTexture.right-pColorElement->rcTexture.left);
					rcDraw.bottom = rcSel.top+(pColorElement->rcTexture.bottom-pColorElement->rcTexture.top);
					nHOffset = (m_nTextHeight > (pColorElement->rcTexture.bottom-pColorElement->rcTexture.top))?m_nTextHeight:(pColorElement->rcTexture.bottom-pColorElement->rcTexture.top);
					m_pDialog->DrawSprite( pSelElement, &rcDraw );
//> mod
//					m_pDialog->DrawText( pItem->strText, pSelElement, &rcSel );
					m_pDialog->DrawText( pItem->strText, pSelElement, &rcSel, m_bDrawTextShadow );
//					m_pDialog->DrawText( pItem->strText, pColorElement, &rcSel, m_bDrawTextShadow );
//< mod
				}
				else
				{
					...
					nHOffset = (m_nTextHeight > (pColorElement->rcTexture.bottom-pColorElement->rcTexture.top) ) ? m_nTextHeight : (pColorElement->rcTexture.bottom-pColorElement->rcTexture.top);
					m_pDialog->DrawSprite( pColorElement, &rcDraw );
//> mod
//					m_pDialog->DrawText( pItem->strText, pColorElement, &rc );
					m_pDialog->DrawText( pItem->strText, pColorElement, &rc, m_bDrawTextShadow );
//< mod
				}
			}
			else
			{
				...
				if ( nTextHeight < nDrawHeight )
					OffsetRect(&rcTxt, 0, (nDrawHeight/2)-(nTextHeight/2) );
				
				nHOffset = (m_nTextHeight > (pColorElement->rcTexture.bottom-pColorElement->rcTexture.top))?m_nTextHeight:(pColorElement->rcTexture.bottom-pColorElement->rcTexture.top);
				m_pDialog->DrawSprite( pColorElement, &rcDraw );
//> mod
//				m_pDialog->DrawText( pItem->strText, pColorElement, &rcTxt );
				m_pDialog->DrawText( pItem->strText, pColorElement, &rcTxt, m_bDrawTextShadow );
//< mod
			}

			OffsetRect( &rc, 0, nHOffset );
        }
    }
...
{
	DXUTControlLock l(&s_bIMEEditBoxThreadSafe, &s_IMEEditBoxCS);
	l.DXUTControlLockDestroy(&s_bIMEEditBoxThreadSafe, &s_IMEEditBoxCS);
}
...
void CDXUTIMEEditBox::UpdateRects()
{
    ...
    SetRect( &m_rcIndicator, m_rcBoundingBox.right, m_rcBoundingBox.top, m_x + m_width + m_nBorder, m_rcBoundingBox.bottom );
...
// Meter control
//-----------------------------------------------------------------------------
CDXUTMeter::CDXUTMeter( CDXUTDialog *pDialog)
{
    m_Type = DXUT_CONTROL_METER;
    m_pDialog = pDialog;
	...
	m_eMeterStyle = CDXUTMeterStyleLeftToRight;

    m_Elements.RemoveAll();
}

void CDXUTMeter::UpdateRects()
{
	CDXUTControl::UpdateRects();
	...
	{
	case CDXUTMeterStyleLeftToRight:
		nMax = (m_width - m_nMarginW*2);
		nWidth = (nMax>0)?(int)((float)nMax*fp):0;
		nHeight = m_height - m_nMarginH*2;
		SetRect(&m_rcValue,
			m_nMarginW+m_rcBoundingBox.left,
			m_nMarginH+m_rcBoundingBox.top,
			nWidth+m_nMarginW+m_rcBoundingBox.left,
			nHeight+m_nMarginH+m_rcBoundingBox.top);
		SetRect(&m_rcDraw,
			pElement->rcTexture.left,
			pElement->rcTexture.top,
			pElement->rcTexture.left+	RectWidth(pElement->rcTexture)*fp,
			pElement->rcTexture.bottom
			);
		break;
	case CDXUTMeterStyleBottomToTop:
		nMax = (m_height - m_nMarginH*2);
		nWidth = m_width - m_nMarginW*2;
		nHeight = (nMax>0)?(int)((float)nMax*fp):0;	
		SetRect(&m_rcValue,
			m_nMarginW+m_rcBoundingBox.left,
			m_nMarginH+m_rcBoundingBox.top+nMax-nHeight,
			nWidth+m_nMarginW+m_rcBoundingBox.left,
			nMax+m_nMarginH+m_rcBoundingBox.top);
		SetRect(&m_rcDraw,
			pElement->rcTexture.left,
			pElement->rcTexture.bottom-RectHeight(pElement->rcTexture)*fp,
			pElement->rcTexture.right,
			pElement->rcTexture.bottom
			);
		break;
	}
	CopyRect(&m_rcValueTexture, &pElement->rcTexture);

}

void CDXUTMeter::Render( IDirect3DDevice9* pd3dDevice, float fElapsedTime )
{
    ...
    //TODO: remove magic numbers
    CDXUTElement* pElement = m_Elements.GetAt( 0 );
    pElement->TextureColor.Blend( iState, fElapsedTime, 0.8f );
	...
	if (m_nValue)
	{
	    ...
	    // Blend current color
	    pElement->TextureColor.Blend( iState, fElapsedTime, 0.8f );
		CopyRect(&pElement->rcTexture, &m_rcDraw);

		m_pDialog->DrawSprite( pElement, &m_rcValue );
		CopyRect(&pElement->rcTexture, &m_rcValueTexture);
	}
...
void CDXUTMeter::Render(ID3DXSprite* pSprite, IDirect3DDevice9* pd3dDevice, CDXUTDialog* pDialog, float fElapsedTime, D3DXMATRIX* mat )
{
	if (!pSprite) return;
    if( m_bVisible == false )        return;
    DXUT_CONTROL_STATE iState = DXUT_STATE_NORMAL;
    ...
    RECT rcWindow = m_rcBoundingBox;
    pDialog->DrawSprite( pSprite, pElement, pDialog, &m_rcBoundingBox, mat );
	...
	if (m_nValue)
	{
		...
		m_pDialog->DrawSprite( pSprite, pElement, pDialog, &m_rcValue, mat );
		CopyRect(&pElement->rcTexture, &m_rcValueTexture);
	}
	// Frame
	pElement = m_Elements.GetAt( 2 );
...
//> mod
CDXUTAngleLine::CDXUTAngleLine(CDXUTDialog* pDialog)
{
    m_Type = DXUT_CONTROL_ANGLELINE;
    m_pDialog = pDialog;

	m_nLength = 0;
	m_nAngle = 0;
	ZeroMemory(&line, sizeof(POINT)*2);
	m_clrLineColor = 0xFFFFFFFF;
...
void CDXUTAngleLine::UpdateRects()
{
	CDXUTControl::UpdateRects();
	// 線始まり
	line.m_pntStart.x = m_width?(m_width / 2)+m_x:0;
	line.m_pntStart.y = m_height?(m_height / 2)+m_y:0;
	// 角度をラジアン
	double dAngle = D3DX_PI * 2 * m_nAngle / 360;
	
	// 線終わり
	line.m_pntEnd.x = (int)(cos(dAngle) * (float)m_nLength) + line.m_pntStart.x;	
	line.m_pntEnd.y = (int)(sin(dAngle) * (float)m_nLength) + line.m_pntStart.y;

}

void CDXUTAngleLine::SetAngle(int x,int y)
{
	double dx,dy;
	double s;
	int deg;
	dx = x - line.m_pntStart.x;
	dy = y - line.m_pntStart.y;

	s= acos(x/sqrt(dx*dx+dy*dy)); /*角度θを求める*/
...
void CDXUTAngleLine::Render( IDirect3DDevice9* pd3dDevice, float fElapsedTime )
{
    if( m_bVisible == false )        return;
    DXUT_CONTROL_STATE iState = DXUT_STATE_NORMAL;
    ...
    // Line
	if (m_nLength)
		m_pDialog->DrawPolyLine((POINT*)&line, 2, m_clrLineColor);
...
//--------------------------------------------------------------------------------------
CDXUTSprite::CDXUTSprite( CDXUTDialog *pDialog )
{
    m_Type = DXUT_CONTROL_SPRITE;
    m_pDialog = pDialog;
/*
	m_nMouserOverOffsetX = 0;
	m_nMouserOverOffsetY = 0;
	m_nPressedOffsetX = 0;
	m_nPressedOffsetY = 0;
*/
...
void CDXUTSprite::Render( ID3DXSprite* pSprite, IDirect3DDevice9* pd3dDevice, CDXUTDialog* pDialog, float fElapsedTime, D3DXMATRIX* mat )
{
	if (!pSprite) return;
    if( m_bVisible == false )        return;
    DXUT_CONTROL_STATE iState = DXUT_STATE_NORMAL;

//	D3DXMATRIX matWorld = m_matLocal* (*mat);
...
void CDXUTSprite::Render( IDirect3DDevice9* pd3dDevice, float fElapsedTime )
{
    if( m_bVisible == false )        return;
    DXUT_CONTROL_STATE iState = DXUT_STATE_NORMAL;
...
//	D3DXMATRIX matSave;
//	pd3dDevice->GetTransform(D3DTS_TEXTURE0, &matSave);
//	D3DXMATRIX matWorld = m_matLocal * matSave;
//	pd3dDevice->SetTransform(D3DTS_TEXTURE0, &matWorld);

	//    RECT rcWindow = m_rcBoundingBox;
    m_pDialog->DrawSprite( pElement, &m_rcBoundingBox );
	
//	pd3dDevice->SetTransform(D3DTS_TEXTURE0, &matSave);
}
//< mod

CDXUTAnimationSprite::CDXUTAnimationSprite( CDXUTDialog *pDialog)
{
	m_Type = DXUT_CONTROL_ANIMATION_SPRITE;
    m_pDialog = pDialog;
	m_bLoop = FALSE;

	m_bAutoCountOnRender = TRUE;
	Clear();
...
void CDXUTAnimationSprite::Render( IDirect3DDevice9* pd3dDevice, float fElapsedTime )
{
    if( m_bVisible == false )        return;
	if (m_vecRcTexture.empty() ) return;
	...
	if (m_bEnabled && m_bAutoCountOnRender)
		StepTimeCount();
