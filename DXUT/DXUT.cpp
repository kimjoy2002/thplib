...
//> mod
		bool	m_InsideDrawloop;
		bool	m_InsideFrameloop;
//< mod
...
//> mod
    GET_SET_ACCESSOR( bool, InsideFrameloop );
    GET_SET_ACCESSOR( bool, InsideDrawloop );
//< mod
...
//> mod
bool DXUTIsDeviceLost()								{ return GetDXUTState().GetDeviceLost();	}
//< mod
...
//    DXUTUpdateFrameStats();
    ...
    if( !GetDXUTState().GetRenderingPaused() )
    {
        ...
        if( pCallbackFrameRender != NULL )
        {
            pCallbackFrameRender( pd3dDevice, fTime, fElapsedTime, &hr );
            pd3dDevice = DXUTGetD3DDevice();
            if( NULL == pd3dDevice ) // Handle DXUTShutdown from inside callback
                return;
        }
...
// has not already been called, it will call DXUTCreateWindow() with the default parameters.  
//--------------------------------------------------------------------------------------
HRESULT DXMyMainLoop( HACCEL hAccel )
{
    HRESULT hr;
...
HRESULT DXMyMessageLoop( HACCEL hAccel )
{
//    HWND hWnd = DXUTGetHWND();
    // Now we're ready to receive and process Windows messages.
    MSG  msg;
...
// has not already been called, it will call DXUTCreateWindow() with the default parameters.  
//--------------------------------------------------------------------------------------
HRESULT DXMyDrawLoop( DWORD dwDrawRate, HACCEL hAccel )
{
    HRESULT hr;
    // Not allowed to call this from inside the device callbacks or reenter
    if( GetDXUTState().GetInsideDeviceCallback() || GetDXUTState().GetInsideDrawloop() )
    {
        if( (GetDXUTState().GetExitCode() == 0) || (GetDXUTState().GetExitCode() == 11) )
            GetDXUTState().SetExitCode(1);
        return DXUT_ERR_MSGBOX( L"DXUTFrameLoop", E_FAIL );
    }

    GetDXUTState().SetInsideDrawloop( true );
...
// has not already been called, it will call DXUTCreateWindow() with the default parameters.  
//--------------------------------------------------------------------------------------
HRESULT DXMyFrameLoop( DWORD dwRunRate, HACCEL hAccel )
{
    HRESULT hr;
    // Not allowed to call this from inside the device callbacks or reenter
    if( GetDXUTState().GetInsideDeviceCallback() || GetDXUTState().GetInsideFrameloop() )
    {
        if( (GetDXUTState().GetExitCode() == 0) || (GetDXUTState().GetExitCode() == 11) )
            GetDXUTState().SetExitCode(1);
        return DXUT_ERR_MSGBOX( L"DXUTFrameLoop", E_FAIL );
    ...
    GetDXUTState().SetInsideFrameloop( true );
    ...
    while( WM_QUIT != msg.message  )
    {
		...
		{													// 指定時間異常経過ならゲーム処理開始
			...
			{
				float fElapsedTime  = (float) DXUTGetGlobalTimer()->GetElapsedTime();
				double fTime        = DXUTGetGlobalTimer()->GetTime()+fElapsedTime;

				pd3dDevice = DXUTGetD3DDevice();
				if( NULL == pd3dDevice ) // Handle DXUTShutdown from inside callback
				{
					if( GetDXUTState().GetDeviceLost() )
						Sleep(10);											// CPU に休みを入れる
					continue;
				}
				pCallbackFrameMove( pd3dDevice, fTime, fElapsedTime, GetDXUTState().GetFrameMoveFuncUserContext() );
			}
