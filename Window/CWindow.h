...
private:
	...
	HINSTANCE	m_hInstance;								// インス?ンスハンドル
	HWND		m_hWnd;										// ウィンドウハンドル
	ATOM		m_Atom;										// ウィンドウクラス登?時のATOM
	TCHAR*		m_pszClassName;								// クラス名

	LONG		m_nXsize, m_nYsize;							// ウィンドウサイズ

	P_RENDER	m_pFuncUserRender;							// クラス外部の?画関数?イン?
	P_KEYDOWN	m_pFuncUserKeyDown;							// クラス外部のキ?情報取得関数?イン?
	...
	// [静的メンバ] オブジェクト毎に処理を振り分けるための静的プロシ?ジャ
