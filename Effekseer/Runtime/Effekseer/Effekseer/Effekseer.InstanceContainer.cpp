...
//
//----------------------------------------------------------------------------------
void InstanceContainer::Update( bool recursive )
{
	// 更新
...
void InstanceContainer::Draw( bool recursive )
{
	...
	{
		m_pEffectNode->BeginRendering();

		// 描画
		for( InstanceGroup* group = m_headGroups; group != NULL; group = group->NextUsedByContainer )
		{
			m_pEffectNode->BeginRenderingGroup( group->GetInstanceCount() );
